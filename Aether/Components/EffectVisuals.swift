...
struct EffectFamiliesView: View {
    ...
    private let rows: [Row] = [
        ...
        Row(name: "Reverb",    gain: true,  delay: true,  how: "thousands of copies, each quieter"),
    ]
        ...
        VStack(spacing: 8) {
            ...
            VStack(spacing: 0) {
                ForEach(rows, id: \.name) { r in
                    HStack(spacing: 8) {
                        Text(r.name).mono(10, .semibold)
                            .foregroundColor(Theme.textPrimary)
                            .frame(width: 74, alignment: .leading)
                        // Two dots rather than a word, so the pattern down the column reads at a
                        // glance: every row has at least one filled, and none has a third.
                        dot(on: r.gain, color: accent)
                        dot(on: r.delay, color: Theme.textPrimary)
                        Text(r.how).mono(9).foregroundColor(Theme.textMuted)
                            .frame(maxWidth: .infinity, alignment: .leading)
                    }
                    .padding(.vertical, 5)
                    if r.name != rows.last?.name {
            ...
            Text("No third column. Every box on the shelf is somewhere in this table.")
                .mono(9).foregroundColor(Theme.textMuted)
                .multilineTextAlignment(.center).frame(maxWidth: .infinity)
        }
    }
    ...
    private func key(color: Color, label: String) -> some View {
        HStack(spacing: 5) {
            ...
            Text(label).mono(9).foregroundColor(Theme.textMuted)
        }
    }

    private func dot(on: Bool, color: Color) -> some View {
        Circle()
            .fill(on ? color : Color.clear)
            .overlay(Circle().stroke(on ? color : Theme.hairline(0.25), lineWidth: 1))
